...
export class PaymentsClient {
  ...
  constructor(opts: PaymentsOpts) {
    ...
    this.#keeper = opts.keeperSecretBase58
      ? Keypair.fromSecretKey(bs58.decode(opts.keeperSecretBase58))
      : null;
  }
  ...
  async registerUserAgent(args: {
    ...
    console.log(
      ...
      JSON.stringify({
        ...
        protocolsMask,
        pricing,
        keeper: this.#keeper?.publicKey.toBase58() ?? null,
      }),
    );
  ...
  async mintMandate(args: MandateArgs): Promise<{ jwt: string; exp: number }> {
    const key = await importPKCS8(normalizePrivyPrivateKey(this.#opts.privyPrivateKeyPem), 'ES256');
...
function methodHashHex(method: string): string {
  ...
  // FNV-1a placeholder — replace with SHA-256 first 8 bytes to match
