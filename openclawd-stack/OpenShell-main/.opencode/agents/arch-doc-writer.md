---
description: >
  Use this agent when documentation in the `architecture/` directory needs to be
  updated or created after implementing a feature, fix, refactor, or behavior
  change. Launch one instance per file that needs updating. Maintains the
  contents of architecture documentation files - does not decide which files
  exist or how the directory is organized.
mode: subagent
color: "#e6b800"
tools:
  bash: false
