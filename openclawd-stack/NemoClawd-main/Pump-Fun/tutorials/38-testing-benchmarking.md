...
echo "$PUBKEY" | grep -q "^${PREFIX}" && echo "✅ Prefix match" || echo "❌ Prefix mismatch"
```
...
test_cases = [
    ...
    ("<script>alert(1)</script>", "XSS", True),
    ("{{7*7}}", "Template injection", True),

    # Unicode and special characters
    ("🚀Pump", "Emoji prefix", True),
...
- See [Tutorial 31](./31-rust-vanity-deep-dive.md) for Rust generator internals
- See [Tutorial 30](./30-batch-shell-scripts.md) for batch generation scripts
