/**
 * PumpKit Monitor — Groq AI Integration
 *
 * Optional LLM summarization using Groq's fast inference API.
 * Generates concise one-line AI summaries for token alerts.
 *
 * Degrades gracefully if GROQ_API_KEY is not set.
...
let lastCallMs = 0;
const MIN_INTERVAL_MS = 2_000; // Max ~30 req/min to stay under free tier
...
function getApiKey(): string {
    ...
    if (!key && !_loggedDisabled) {
        log.info('Groq integration disabled — set GROQ_API_KEY to enable AI summaries');
...
/**
 * Generate a concise 1-line AI summary for a token alert.
 * Returns empty string if Groq is unavailable, rate-limited, or fails.
 */
...
- "Graduated token, 50+ holders, dev self-claimed 2.6 SOL — active project"`,
        ...
        // Strip any HTML tags from AI output for safety
...
function buildFactString(input: SummaryInput): string {
    const lines: string[] = [];
