...
description: "Review and approve blocked NemoClawd agent network requests in the OpenShell TUI."
keywords: ["nemoclawd approve network requests", "sandbox egress approval tui", "openclawd-stack network policy"]
...
Review and act on network requests that the NemoClawd agent makes to endpoints not listed in the sandbox policy.
...
When the NemoClawd agent attempts to reach an endpoint that is not in the baseline policy, OpenShell blocks the connection and displays the request in the TUI.
...
This script opens a split tmux session with the TUI on the left and the NemoClawd agent on the right.
The walkthrough requires tmux and either the `XAI_API_KEY` or `NVIDIA_API_KEY` environment variable.
