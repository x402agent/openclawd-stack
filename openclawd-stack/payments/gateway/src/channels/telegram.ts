// gateway/src/channels/telegram.ts — adapted for payments.
//
// Diff summary vs the original:
//   + AttachArgs.payments — SandboxPayments is wired so the channel can expose
//     /balance, /spend, /pay commands.
//   + Command handlers added before the generic session-send path:
...
//   + Paying agents get SandboxPayments via createSession so if the bot spawns
//     a fresh session it's already payment-aware.
...
export function attachTelegram({
  ...
  const handleUpdate = async (u: TelegramUpdate) => {
    ...
    if (text === '/mandate') {
      ...
      try {
        const payload = JSON.parse(Buffer.from(jwt.split('.')[1], 'base64url').toString()) as {
          exp: number; maxAmount: string; resource: string;
        };
        ...
        await api('sendMessage', { chat_id: msg.chat.id, text: 'Clawd: mandate present but unreadable.' });
      }
      return;
    }
    ...
    if (text.startsWith('/pay-agent ')) {
      ...
      try {
        ...
        await api('sendMessage', { chat_id: msg.chat.id, text: `Clawd: ${(err as Error).message}` });
      }
      return;
    }
  ...
  (globalThis as { __clawdTelegram?: unknown }).__clawdTelegram = {
    listPending: () => Array.from(pendingCodes.entries()).map(([code, uid]) => ({ code, uid })),
