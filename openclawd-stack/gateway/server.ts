// Gateway server — runs INSIDE the E2B sandbox. Exposes an HTTP + WebSocket
// surface that the orchestrator (solanaclawd.com backend) proxies to the
// user's browser, or that Telegram hits directly via the channel.
//
// Auth modes:
//   - privy: verify Privy JWT via JWKS (production default)
//   - token: legacy bearer (?token=) — dev only, matches openclawd quick-start
//   - both:  accept either, prefer Privy
...
const log = pino({
  level: process.env.CLAWD_LOG_LEVEL ?? 'info',
  name: 'clawd-gateway',
});
...
// Orchestrator-driven Telegram pairing approval. The CLI / solanaclawd.com
// backend calls this after the user DMs the bot and receives a code.
/* ——— brain (Honcho peer.chat over the user's own history) ——— */
...
app.post('/v1/brain/ask', async (c) => {
  ...
  const answer = await vault.brainAsk(sub.sub, body.query.trim(), body.agent);
  return c.json({ answer, queriedAt: Date.now() });
});
...
// ── WS surface — streaming tokens + tool calls to the browser ──────────
const server = serve(
  { fetch: app.fetch, port: PORT, hostname: '0.0.0.0' },
  (info) => {
    log.info({ port: info.port }, 'gateway listening');
  },
);
...
// ── Channels (Telegram, future: Discord, Farcaster) ────────────────────
if (process.env.TELEGRAM_BOT_TOKEN) {
  attachTelegram({
    token: process.env.TELEGRAM_BOT_TOKEN,
    vault,
    payments,
    // Chat allowlist — -1003338091119 is the default control room
...
// Graceful shutdown — flush vault before exit (orchestrator-initiated)
